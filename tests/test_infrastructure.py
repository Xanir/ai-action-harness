...
from src.mcp_bridge import MCPRegistry, validate_action_dependencies


def _make_registry(*server_names: str) -> MCPRegistry:
    """Build a minimal :class:`MCPRegistry` for testing."""
    servers = {
        name: {"transport": "sse", "url": f"http://localhost/{name}"}
...
class TestValidateActionDependencies:
    ...
    def test_all_required_present_and_healthy(self):
        """Should pass silently when every registered server is up."""
        status = {
            "codegraph_mcp": (True, ["tool_a"]),
            "jira_mcp": (True, ["tool_b"]),
        }
        validate_action_dependencies(
            _make_registry("codegraph_mcp", "jira_mcp"), status
        )

    def test_required_server_down_raises(self):
        """A down registered server must raise RuntimeError."""
        status = {
            "codegraph_mcp": (False, None),
            "jira_mcp": (True, ["tool_b"]),
        }
        with pytest.raises(RuntimeError, match="codegraph_mcp"):
            validate_action_dependencies(_make_registry("codegraph_mcp"), status)

    def test_multiple_required_servers_down(self):
        """All failing registered servers are named in the error."""
        status = {
            "codegraph_mcp": (False, None),
            "jira_mcp": (False, None),
        }
    ...
    def test_optional_down_only_warns(self, caplog):
        """An unavailable server *not* in the registry must not raise."""
        import logging
    ...
    def test_required_not_in_status_at_all_raises(self):
        """A registered server absent from the check results must fail fast."""
        status = {"codegraph_mcp": (True, ["tool_a"])}
        with pytest.raises(RuntimeError, match="jira_mcp"):
            validate_action_dependencies(_make_registry("jira_mcp"), status)
